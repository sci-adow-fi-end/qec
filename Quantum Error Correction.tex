...
When the system is entangled with the environment it is desirable to
have a way to represent how the system changes when some kind of
transformation is applied to it.
Suppose the system starts in an initial state $\rho$ and, after it gets
inevitably entangled with the environment, that starts in state $\rho_{env}$,
